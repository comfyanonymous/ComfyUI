...
def to_json(flow):
    flow = flow.to_mongo().to_dict()
    print(flow)
    flow["_id"] = str(flow["_id"])
    return flow


@AIYoApiServer.instance.routes.get("/open_api/services")
async def get_paged_flows(request):
    page = int(request.rel_url.query["page"]) or 1
    page_size = int(request.rel_url.query["page_size"]) or 10
    AppLog.info(
        "[API] get paged flows at page {page} with count {page_size}")

    flows = tb_data.Flow.objects().skip(page_size * (page - 1)).limit(page_size)
    count = tb_data.Flow.objects().count()
    flows = [flow for flow in map(to_json, [flow for flow in flows])]
...
async def upload_file(request):
    ...
    try:
        # upload
        remote_name = folder_paths.input_path_local_to_remote(
            file_field.filename, rename=True)
        file_data = file_field.file.read()
        object_storage.MinIOConnection().put_object(
            remote_name, io.BytesIO(file_data), len(file_data))
        AppLog.info(f"[API] upload resource, result: {remote_name}")
    except Exception as e:
        AppLog.info(
            f"[API] upload file, unexpected error. \n{traceback.format_exc()}")
...
async def run_flow(request):
    ...
    try:
        ...
        AppLog.info(
            f"[API] run_flow, json data: {AppLog.visible_convert(json_data)} ")
    except Exception as e:
...
async def register_webhook(request):
    ...
    if code == ErrorCode.SUCCESS:
        ...
        code, err = ServerHelper.register_webhook(
            flow_id=flow_id, on_start=on_start, on_processing=on_processing, on_end=on_end)
