...
from . import deis, sa_solver, utils
...
def _apply_dynamic_s_noise(dynamic_opts, model_sampling, current_s_noise):
    """Re-read s_noise from the mutable extra_options dict if available.

    Bounded-feedback callbacks update ``extra_options["s_noise"]`` at each step.
    Call this at the top of each sampler loop iteration with *dynamic_opts*
    set to the KSAMPLER.extra_options reference (popped from extra_args so it
    never reaches the model).
    """
    if dynamic_opts is None:
        return current_s_noise
    new_val = dynamic_opts.get("s_noise")
    if new_val is None:
        return current_s_noise
    noise_scale = getattr(model_sampling, "noise_scale", 1.0)
    return new_val * noise_scale


def _init_dynamic_options(extra_args):
    """Pop and return the mutable extra_options dict for per-step re-reading,
    or None if no bounded-feedback is active on this sampler."""
    if extra_args is None:
        return None
    return extra_args.pop("_dynamic_sampler_options", None)


def _refresh_dynamic_params(dynamic_opts, model_sampling, s_noise, eta):
    """Re-read s_noise and eta from mutable dynamic_opts if available.
    Returns (s_noise, eta) tuple with updated values.
    """
    if dynamic_opts is None:
        return s_noise, eta
    ns = getattr(model_sampling, "noise_scale", 1.0)
    if "s_noise" in dynamic_opts:
        s_noise = dynamic_opts["s_noise"] * ns
    if "eta" in dynamic_opts:
        eta = dynamic_opts["eta"]
    return s_noise, eta
...
def sample_euler_ancestral(
    ...
    extra_args = {} if extra_args is None else extra_args
    _dynamic_opts = _init_dynamic_options(extra_args)
    ...
    s_in = x.new_ones([x.shape[0]])
    for i in trange(len(sigmas) - 1, disable=disable):
        if _dynamic_opts is not None:
            s_noise = _dynamic_opts.get("s_noise", s_noise)
            eta = _dynamic_opts.get("eta", eta)
...
def sample_euler_ancestral_RF(
    ...
    s_in = x.new_ones([x.shape[0]])
    for i in trange(len(sigmas) - 1, disable=disable):
        s_noise, eta = _refresh_dynamic_params(
            _dynamic_opts, model_sampling, s_noise, eta
        )
        denoised = model(x, sigmas[i] * s_in, **extra_args)
        # sigma_down, sigma_up = get_ancestral_step(sigmas[i], sigmas[i + 1], eta=eta)
        if callback is not None:
...
def sample_dpmpp_sde(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        if _dynamic_opts is not None and "r" in _dynamic_opts:
            r = _dynamic_opts["r"]
        denoised = model(x, sigmas[i] * s_in, **extra_args)
        if callback is not None:
            callback(
...
def sample_er_sde(
    ...
    """
    extra_args = {} if extra_args is None else extra_args
    # Pop bounded-feedback dynamic options so they never reach the model call.
    # We keep a reference to the same mutable dict — the callback updates it
    # in-place and we re-read at each loop iteration.
    _dynamic_opts = extra_args.pop("_dynamic_sampler_options", None)
    seed = extra_args.get("seed", None)
    noise_sampler = (
        default_noise_sampler(x, seed=seed) if noise_sampler is None else noise_sampler
    )
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        s_noise = _apply_dynamic_s_noise(_dynamic_opts, model_sampling, s_noise)
        denoised = model(x, sigmas[i] * s_in, **extra_args)
        if callback is not None:
            callback(
                {
...
def sample_sa_solver(
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        # Re-read dynamic s_noise updated per-step by bounded-feedback.
        s_noise = _apply_dynamic_s_noise(_dynamic_opts, model_sampling, s_noise)
...
def sample_ar_video(
    ...
    if not (
        hasattr(causal_model, "init_kv_caches")
        and hasattr(causal_model, "init_crossattn_caches")
    ):
        raise TypeError(
    ...
    kv_caches = causal_model.init_kv_caches(
        bs, lat_t * frame_seq_len, device, model_dtype
    )
    ...
    initial_latent = transformer_options.get("ar_config", {}).get(
        "initial_latent", None
    )
    if initial_latent is not None:
        initial_latent = inner_model.process_latent_in(initial_latent).to(
        ...
        for block_idx in trange(num_blocks, disable=disable):
            ...
            for i in range(num_sigma_steps):
                ...
                if sigmas[i + 1] == 0:
                    ...
                    noisy_input = (
                        1.0 - sigma_next
                    ) * denoised + sigma_next * fresh_noise
