...
class ModelPatcher:
    ...
    def unpatch_model(self, device_to=None, unpatch_weights=True):
        ...
        if unpatch_weights:
            ...
            if device_to is not None:
                modules_to_move = [ weakref.ref(m[2]) for m in self._load_list() ]
    ...
    def partially_load(self, device_to, extra_memory=0, force_patch_weights=False):
        with self.use_ejected(skip_and_inject_on_exit_only=True):
            ...
            if extra_memory < 0 and not unpatch_weights:
                modules_to_offload = self.partially_unload(self.offload_device, -extra_memory, force_patch_weights=force_patch_weights)
                comfy.model_management.offload_modules(modules_to_offload, self.offload_device)
                return 0
            full_load = False
