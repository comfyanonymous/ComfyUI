...
class SAM3TokenizerWrapper(sd1_clip.SD1Tokenizer):
    ...
    def tokenize_with_weights(self, text: str, return_word_ids=False, **kwargs):
        ...
        if len(parsed) <= 1 and (not parsed or parsed[0][1] == 1):
            # Use the parsed phrase (with any ":N" suffix already stripped), not
            # the raw text -- otherwise "person:1" is encoded as the literal
            # string "person:1" instead of "person", even though _parse_prompts
            # already determined max_detections=1 for it.
