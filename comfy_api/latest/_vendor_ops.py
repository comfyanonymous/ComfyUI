...
async def image_classifier_classify(classifier: "ImageClassifierRef", images: "ImageRef",
...
async def image_classifier_predict_scores(classifier: "ImageClassifierRef", images: "ImageRef",
) -> "ClassifierScoresRef":
    import numpy as np
    import torch
    from PIL import Image
    ...
    return _sdk.ClassifierScoresRef._wrap(await rt.refs.create(
        "CLASSIFIER_SCORES", value))  # type: ignore[return-value]

async def classifier_scores_shape(scores: "ClassifierScoresRef",
) -> tuple[int, int]:
    import numpy as np

    bundle = await _sdk.current_runtime().refs.resolve(scores)
...
async def classifier_scores_select_above(scores: "ClassifierScoresRef", batch_index: int,
