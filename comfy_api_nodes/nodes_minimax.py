...
FAL_MINIMAX_VIDEO = "fal-ai/minimax/video-01-director"
...
async def _generate_mm_video(
    ...
    *,
    ...
    if subject is not None:
        subject_url = (await upload_images_to_fal(subject, max_images=1))[0]
    ...
    data = {
        ...
        "subject_reference": [s.model_dump() for s in subject_reference] if subject_reference else None,
    }
    result = await fal_run(
        cls, FAL_MINIMAX_VIDEO, data,
...
class MinimaxHailuoVideoNode(IO.ComfyNode):
    ...
    async def execute(
        ...
        if first_frame_image is not None:
            image_url = (await upload_images_to_fal(first_frame_image, max_images=1))[0]
