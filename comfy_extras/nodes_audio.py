...
class LoadAudio:
    ...
    def get_supported_formats(s):
        ...
        if "soundfile" in available_backends:
            soundfile_formats = ['.aiff', '.au', '.avr', '.caf', '.flac', '.htk', '.svx', '.mat4', '.mat5', '.mpc2k', '.mp3', '.ogg', '.paf', '.pvf', '.raw', '.rf64', '.sd2', '.sds', '.ircam', '.voc', '.w64', '.wav', '.nist', '.wavex', '.wve', '.xi']
            supported.update(soundfile_formats)
        if "sox_io" in available_backends or "sox" in available_backends:
        ...
        if "ffmpeg" in available_backends:
            ...
            supported.update(ffmpeg_formats)
