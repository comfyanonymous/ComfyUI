    ...
    def define_schema(cls):
        ...
        return io.Schema(
            ...
            inputs=[
                ...
                io.MatchType.Input("initial_value", carried_type, optional=True),
            ],
            outputs=[
                ...
                io.MatchType.Output(carried_type, id="current_value"),
            ],
            hidden=[io.Hidden.dynprompt, io.Hidden.execution_list, io.Hidden.unique_id],
        )

    @classmethod
    def execute(cls, mode, iteration_outer=None, initial_value=None):
        ...
        if state is None:
            ...
            def source_values(source, input_name):
                ...
                if cached is None:
                    raise RuntimeError(
                        f"Close Loop {state['close_id']} {input_name} was not produced during the iteration"
                    )
...
class CloseLoop(io.ComfyNode):
    ...
    def define_schema(cls):
        ...
        terminations = io.Autogrow.TemplatePrefix(
            input=io.AnyType.Input("termination"),
            prefix="termination",
            min=0,
            max=50,
        )
        return io.Schema(
            ...
            outputs=[
                io.MatchType.Output(output_type, id="output", is_output_list=True),
            ],
