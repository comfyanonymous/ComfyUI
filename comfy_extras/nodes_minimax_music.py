...
from comfy.ldm.minimax_music.ar import CFG_SCALE, CFG_TOP_K, C0_VOCAB_SIZE, MAX_AUDIO_FRAMES
...
class MiniMaxMusic3TextEncode(io.ComfyNode):
    ...
    def execute(cls, clip, caption, lyrics, seed, max_duration, cfg_scale, top_k):
        ...
        return io.NodeOutput(conditioning, conditioning[0][0].shape[1] / 25)
...
class EmptyMiniMaxMusic3LatentAudio(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                io.Float.Input("seconds", default=120.0, min=0.04, max=MAX_AUDIO_FRAMES / 25, step=0.04),
    ...
    def execute(cls, seconds, batch_size):
        audio_frames = min(MAX_AUDIO_FRAMES, max(1, round(seconds * 25)))
        latent = torch.zeros(
