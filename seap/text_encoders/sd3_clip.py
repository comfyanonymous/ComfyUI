from seap import sd1_clip
from seap import sdxl_clip
from transformers import T5TokenizerFast
import seap.text_encoders.t5
import torch
import os
...
class T5XXLModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="last", layer_idx=None, dtype=None, attention_mask=False, model_options={}):
        ...
        super().__init__(device=device, layer=layer, layer_idx=layer_idx, textmodel_json_config=textmodel_json_config, dtype=dtype, special_tokens={"end": 1, "pad": 0}, model_class=seap.text_encoders.t5.T5, enable_attention_masks=attention_mask, return_attention_masks=attention_mask, model_options=model_options)
...
class SD3ClipModel(torch.nn.Module):
    def __init__(self, clip_l=True, clip_g=True, t5=True, dtype_t5=None, t5_attention_mask=False, device="cpu", dtype=None, model_options={}):
        ...
        if t5:
            dtype_t5 = seap.model_management.pick_weight_dtype(dtype_t5, dtype, device)
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            if self.clip_l is not None:
                ...
                l_pooled = torch.zeros((1, 768), device=seap.model_management.intermediate_device())
            ...
            if self.clip_g is not None:
                ...
                g_out = None
                g_pooled = torch.zeros((1, 1280), device=seap.model_management.intermediate_device())
        ...
        if out is None:
            out = torch.zeros((1, 77, 4096), device=seap.model_management.intermediate_device())

        if pooled is None:
            pooled = torch.zeros((1, 768 + 1280), device=seap.model_management.intermediate_device())
