...
class TestAsyncNodes:
    ...
    def test_async_sibling_completes_after_recoverable_error(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableAsyncError", value=image.out(0), error_after=0.05)
        sleep_node = g.node("TestSleep", value=image.out(0), seconds=0.1)
        g.node("PreviewImage", images=error_node.out(0))
    ...
    def test_async_generic_error_fails_fast_with_sibling(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_async_sibling_completes_after_multiple_recoverable_errors(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error1 = g.node("TestRecoverableAsyncError", value=image.out(0), error_after=0.02)
        error2 = g.node("TestRecoverableAsyncError", value=image.out(0), error_after=0.04)
