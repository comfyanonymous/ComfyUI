...
class TestAsyncNodes:
    ...
    def test_sync_error_during_async_execution(self, client: ComfyClient, builder: GraphBuilder):
        ...
        sleep_output = g.node("PreviewImage", images=sleep_node.out(0))
        g.node("PreviewImage", images=error_node.out(0))

        result = client.run(g)

        # The sync error is reported for its node while the async branch completes
        assert result.did_run(sleep_node)
        assert result.was_executed(sleep_output)
