...
class TestExecution:
    ...
    def test_recoverable_error_continues_independent(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
    ...
    def test_recoverable_error_reuses_failed_node_for_late_lazy_link(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
    ...
    def test_recoverable_error_handles_mixed_dynamic_results(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        zero = g.node("StubInt", value=0)
        one = g.node("StubInt", value=1)
        values = g.node("TestMakeListNode", value1=zero.out(0), value2=one.out(0))
        mixed = g.node("TestMixedExpansionFailure", value=values.out(0), recoverable=True)
    ...
    def test_generic_error_in_expansion_fails_fast(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        zero = g.node("StubInt", value=0)
        one = g.node("StubInt", value=1)
        values = g.node("TestMakeListNode", value1=zero.out(0), value2=one.out(0))
        mixed = g.node("TestMixedExpansionFailure", value=values.out(0), recoverable=False)
        g.node("PreviewImage", images=mixed.out(0))

        with pytest.raises(Exception) as exc_info:
            client.run(g)

        assert exc_info.value.args[0]['node_id'] == mixed.id

    def test_oom_error_remains_terminal(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_recoverable_error_accepts_cached_output(self, client: ComfyClient, builder: GraphBuilder, server):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
        g.node("PreviewImage", images=error_node.out(0))
    ...
    def test_executor_errors_remain_terminal(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_malformed_result_remains_terminal(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_cyclic_expansion_reports_cycle(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_recoverable_error_async_output_partial_failure(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        zero = g.node("StubInt", value=0)
        one = g.node("StubInt", value=1)
        values = g.node("TestMakeListNode", value1=zero.out(0), value2=one.out(0))
    ...
    def test_recoverable_failure_blocker_beats_user_blocker(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
    ...
    def test_recoverable_error_retries_failed_expansion_side_branch(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        expander = g.node("TestExpansionWithFailingOutput", image=image.out(0), recoverable=True)
    ...
    def test_generic_error_in_expansion_side_branch_fails_fast(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        expander = g.node("TestExpansionWithFailingOutput", image=image.out(0), recoverable=False)
        g.node("PreviewImage", images=expander.out(0))

        with pytest.raises(Exception) as exc_info:
            client.run(g)

        assert 'prompt_id' in exc_info.value.args[0]

    def test_recoverable_error_with_partial_targets(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
        ...
        result = client.run(g, partial_execution_targets=[failing_output.id, successful_output.id])
    ...
    def test_generic_error_fails_fast(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
    ...
    def test_recoverable_failure_after_sibling_output(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        fast_output = g.node("TestAsyncOutput", value=image.out(0), seconds=0.05)
        slow_error = g.node("TestRecoverableAsyncError", value=image.out(0), error_after=0.5)
    ...
    def test_recoverable_error_never_stores_failed_outputs_externally(self, client: ComfyClient, builder: GraphBuilder, server):
        ...
        assert delta["TestRecoverableSyncError"] == 0, "Failed node outputs must never reach external cache providers"
    ...
    def test_recoverable_error_fails_when_no_output_survives(self, client: ComfyClient, builder: GraphBuilder):
        g = builder
        image = g.node("StubImage", content="BLACK", height=32, width=32, batch_size=1)
        error_node = g.node("TestRecoverableSyncError", value=image.out(0))
        g.node("PreviewImage", images=error_node.out(0))
