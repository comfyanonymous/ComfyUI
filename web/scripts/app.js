...
export class ComfyApp {
  ...
  async #loadExtensions() {
    ...
    async function recursiveLocalImport(ext) {
        ...
        if (line.trim().startsWith("import")) {
          ...
          if (match) {
            ...
            if (importPath.startsWith("./")) {
              ...
              text = text.replace(line, await recursiveLocalImport(ext));
            } else {
    ...
    const extensionPromises = extensions.map(async (ext) => {
      try {
        // Fetch the file as text - recursively fetching files and replacing the import statements with code
        let text = await recursiveLocalImport(ext);
        let core = `
						  import {$el, ComfyDialog} from '../../scripts/ui.js';
							import {addValueControlWidget, addValueControlWidgets, ComfyWidgets} from '../../scripts/widgets.js';
							import {getLatentMetadata, importA1111, getWebpMetadata, getPngMetadata} from '../../scripts/pnginfo.js';
							import {addDomClippingSetting} from  '../../scripts/domWidget.js';
