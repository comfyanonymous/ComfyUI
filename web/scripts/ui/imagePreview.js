...
export function createImageHost(node) {
	const el = $el("div.ccniy-img-preview");
	let currentImgs;
	let first = true;
	...
	function updateSize() {
		...
		if (currentImgs) {
			...
			if (first) {
				...
				el.style.setProperty("--ccniy-widget-min-height", elH);
			} else {
				el.style.setProperty("--ccniy-widget-min-height", null);
			}
			...
			el.style.setProperty("--ccniy-img-preview-width", w);
			el.style.setProperty("--ccniy-img-preview-height", h);
		}
	}
	return {
		el,
